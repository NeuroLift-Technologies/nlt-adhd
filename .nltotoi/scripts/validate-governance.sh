...
check_content "nltotoi.json"     "NeuroLift-Technologies/nlt-adhd" "Repository name in manifest"
...
echo ""
echo "[ App Decoupling ]"
# Verify src/ does not import archived World/Fusion code (nlt-adhd is app-only, local-first)
# Only flag Python-style from src.* imports for archived modules; src/simulation lab is NOT present in nlt-adhd (local-first, no vendored world)
# For TS, flag any import from archived src paths that would indicate vendoring
if grep -R --include="*.py" --include="*.ts" --include="*.tsx" -E "^\s*(from|import).*src\.(fusion|simulation|avatars|aides|core|database)" src/ 2>/dev/null | head -n 5 | grep -q .; then
  echo "  ❌ DECOUPLING FAILED: src/ still imports archived src (see grep below)"
  grep -R --include="*.py" --include="*.ts" --include="*.tsx" -E "^\s*(from|import).*src\.(fusion|simulation|avatars|aides|core|database)" src/ 2>/dev/null | head -n 10
  ((FAIL++)) || true
else
  echo "  ✅ DECOUPLED: src/ does not import archived src"
  ((PASS++)) || true
fi

# Verify no archive/ directory exists (fresh repo, app-only)
if [[ -d "${REPO_ROOT}/archive" ]]; then
  echo "  ❌ ARCHIVE PRESENT: archive/ must not exist in fresh nlt-adhd (app-only, no archive)"
  ((FAIL++)) || true
else
  echo "  ✅ NO ARCHIVE: archive/ correctly absent (fresh app-only repo)"
  ((PASS++)) || true
fi
