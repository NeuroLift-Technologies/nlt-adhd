 ...
 * 1:20 pipeline: World Engine >> AI-Fusion >> nlt-adhd (20th is RSD Shield per trait catalog; Developer is orchestrator tool in src/orchestrator/developer).
 * This classifier is the orchestrator routing layer (nlt-adhd delivery, local-first, 135M–0.5B LoRA per advocate).
 *
 ...
 *
 * @see https://github.com/NeuroLift-Technologies/neurolift-ai-fusion — trait catalog + SessionOrchestrator/FusionEngine (canonical, not vendored)
 * @see https://github.com/NeuroLift-Technologies/nlt-world-engine — world simulation (upstream, not vendored)
 * @see src/governance/asfdk.ts — ASFDK wrapper (asfdk_assess_text, asfdk_status, asfdk_process_interaction)
 * No archive in nlt-adhd (fresh repo) — canonical lives upstream; local-first, no Cloudflare
 *
...
// Advocate IDs in the 1:20 runtime — 20 personas StayAlert → RSD Shield (per trait catalog).
// MVP v0.1 routes to 6 wired advocates (01,04,05,07,09,20-radShield) via heuristic; remaining 14 are stubs with TODO.
// Developer is NOT trait #20 — it is orchestrator tool at src/orchestrator/developer/.
// Numbers follow original 20 catalog (see neurolift-ai-fusion/src/advocates/ + links.md).
