...
- Pipeline: `World (nlt-world-engine) >> Fusion (neurolift-ai-fusion) >> App (nlt-adhd, 20th is RSD Shield per trait catalog; Developer is orchestrator tool at src/orchestrator/developer)`
...
| 5 | **05 — MemoryMate** | `src/advocates/05-memoryMate/index.ts` | `captureDump(idea, source)` | `encrypted_storage` handling per TOI privacy (`retention: session-only`) via `asfdk_assess_text_sync` + `asfdk_process_interaction_sync`; at-rest base64 obfuscation stub (production: local SQLite encrypted); unsafe → escalate, store flagged marker only |
| 6 | **20 — RSD Shield** | `src/advocates/20-rsdShield/index.ts` | `shieldRsd(input)`, `quickGround()` | `emotional_integrity` preserved check via `asfdk_assess_text_sync` — must NOT manipulate; reframing is `recommendation_only`; if flagged → fallback neutral prompt + `emergency_escalation` |
| 7 | **Developer (orchestrator tool)** | `src/orchestrator/developer/index.ts` | `buildForUser(req)` | `asfdk_review_tool_call` before **any** file/write operation; `approval_required` for `architecture_changes`; requires explicit `approved:true` / human `[Approve]` — no silent auto-patching; `needs_approval` status + governance receipt |
...
Personalization without drift: trait catalog stays canonical in `neurolift-ai-fusion`; app only references via A2A (see `src/orchestrator/classifier.ts`). No simulation code vendored — no `archive/` in nlt-adhd (fresh repo, local-first, no Cloudflare).
